...
class StudentController extends Controller
{
    ...
    public function studentExport()
    {
        ...
        $data['title']                  = 'Students Data Export';
        $page_name                      = 'students-export';
        $data['rows']                   = User::select('student_profiles.*', 'users.email', 'users.phone', 'users.created_at')->join('student_profiles', 'student_profiles.user_id', '=', 'users.id')->where('users.valid', '!=', 3)->where('users.role', '=', 1)->orderBy('users.id', 'DESC')->get();

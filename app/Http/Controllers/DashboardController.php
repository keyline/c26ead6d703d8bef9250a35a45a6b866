                    ...
                    if ($validator->fails()) {
                        ...
                        if ($role == 2) {
                            ...
                            $fields = [
                                ...
                                'edu_institute'     => ((!empty($edu_institute))?json_encode($edu_institute):[]),
                                'edu_title'         => ((!empty($edu_title))?json_encode($edu_title):[]),
                                'edu_year'          => ((!empty($edu_year))?json_encode($edu_year):[]),
                                'work_institute'    => ((!empty($work_institute))?json_encode($work_institute):[]),
                                'work_title'        => ((!empty($work_title))?json_encode($work_title):[]),
                                'work_year'         => ((!empty($work_year))?json_encode($work_year):[]),
                                'award_title'       => ((!empty($award_title))?json_encode($award_title):[]),
                                'award_year'        => ((!empty($award_year))?json_encode($award_year):[]),
                            ...
                            $fields = [
                                ...
                                'social_platform'   => ((!empty($social_platform_array))?json_encode($social_platform_array):[]),
                                'social_url'        => ((!empty($social_link_array))?json_encode($social_link_array):[]),
                                'profile_pic'       => $image,
                                'city'              => $postData['city'],
                                'updated_at'        => date('Y-m-d H:i:s'),
                            ];
                    ...
                    foreach ($days as $key => $value) {
                        ...
                        if (is_array($availabilities['from'][$day_id])) {
                            ...
                            foreach ($availabilities['from'][$day_id] as $key => $value) {
                                ...
                                // Helper::pr($data,0);
                                // DB::enableQueryLog();
                                $checkSameDaySameTimeFromSlotExist = MentorAvailability::where('mentor_user_id', '=', $userId)->where('day_of_week_id', '=', $day_id)->where('avail_from', '=', $avail_from)->where('is_active', '=', 1)->count();
                                ...
                                // echo '<br>';
                                if($checkSameDaySameTimeFromSlotExist <= 0){
                                    $checkSameDaySameTimeToSlotExist = MentorAvailability::where('mentor_user_id', '=', $userId)->where('day_of_week_id', '=', $day_id)->where('avail_to', '=', $avail_to)->where('is_active', '=', 1)->count();
                                    if($checkSameDaySameTimeToSlotExist <= 0){

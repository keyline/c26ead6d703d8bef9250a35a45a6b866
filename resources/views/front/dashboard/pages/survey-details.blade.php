                    ...
                    <div class="col-sm-12 col-lg-12">
                        <form method="POST" action="" enctype="multipart/form-data"
                            ...
                            <input type="hidden" name="question_type"
                                value="<?= $getSurveyDetail ? $getSurveyDetail->question_type : '' ?>">
                            @csrf
                            <div class="survay_listing">
                                <ul>
                                    <?php if($surveyQuestions){ $q=1; foreach($surveyQuestions as $surveyQuestion){	?>
                                    <input type="hidden" name="survey_id" value="<?= $surveyQuestion->survey_id ?>"
                                        id="survey_id">
                                    <li class="input_radio question <?= $q ?> <?= $q == 1 ? '' : 'inactive' ?>"
                                        id="question-container-<?= $q ?>">
                                        <h3><?= $q ?>. <?= $surveyQuestion->question_name ?></h3>
                                        <ul>
                                            <?php $surveyQuestionOptions = surveyQuestionOptions::where('survey_id','=', $surveyQuestion->survey_id)->where('question_id','=',$surveyQuestion->question_id)->where('status','=',1)->get();
												if($surveyQuestionOptions){
                                            ...
                                            <li>
                                                <label for="option-<?= $surveyQuestionOption->option_id ?>">
                                                    <input name="option<?= $q ?>[]"
                                                        value="<?= $surveyQuestionOption->option_id ?>/<?= Helper::encoded($surveyQuestionOption->factor) ?>/<?= Helper::encoded($surveyQuestionOption->option_weight) ?>"
                                                        type="radio"
                                                        id="option-<?= $surveyQuestionOption->option_id ?>"
                                                        onchange="setAnswer(<?= $q ?>);">
                                                    <?= $surveyQuestionOption->option_name ?></label>
                                            </li>
                                            <?php } } ?>
                                        </ul>
                                    </li>
...
<script type="text/javascript">
    ...
    function validateForm() {
        ...
        var radioGroups = $('.input_radio');

        // Iterate through each radio group
        for (var i = 0; i < radioGroups.length; i++) {
            var radioGroup = radioGroups.eq(i);
            var checked = false;

            // Find all radio buttons within the current radio group
            var radioButtons = radioGroup.find('input[type="radio"]');

            // Check if at least one radio button in the group is checked
            for (var j = 0; j < radioButtons.length; j++) {
                if (radioButtons.eq(j).prop('checked')) {
                    checked = true;
                    break; // Exit the loop since we found a checked radio button
                }
            }

            // If no radio button is checked, display an alert and return false
            if (!checked) {
        ...
        // If all questions have at least one radio button checked, return true
        return true;
    }
</script>
